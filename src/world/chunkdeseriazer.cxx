...
void ChunkDeserializer::readBlocks(const std::vector<std::int8_t>& buffer, std::int16_t yy)
{
  const std::uint8_t* ptr = reinterpret_cast<const std::uint8_t*>(buffer.data());
  std::size_t k = 0;
  ...
  for (std::uint8_t y = 0; y < 16; ++y)
  {
    for (std::uint8_t z = 0; z < 16; ++z)
    {
      ...
      {
        chunk(x, y + yy, z).type = static_cast<BlockType>(reinterpret_cast<const std::uint8_t&>(buffer[k++]));
      }
    }
  }
}
...
void ChunkDeserializer::readData(const std::vector<std::int8_t>& buffer, std::int16_t yy)
{
  const std::uint8_t* ptr = reinterpret_cast<const std::uint8_t*>(buffer.data());
  std::size_t k = 0;
...
void ChunkDeserializer::readBlockLight(const std::vector<std::int8_t>& buffer, std::int16_t yy)
{
  const std::uint8_t* ptr = reinterpret_cast<const std::uint8_t*>(buffer.data());
  std::size_t k = 0;
...
void ChunkDeserializer::readSkyLight(const std::vector<std::int8_t>& buffer, std::int16_t yy)
{
  const std::uint8_t* ptr = reinterpret_cast<const std::uint8_t*>(buffer.data());
  std::size_t k = 0;
