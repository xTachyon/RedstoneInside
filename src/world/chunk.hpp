...
public:
  ...
  Block& operator()(std::int32_t x, std::int32_t y, std::int32_t z) { return blocks[x][z][y]; }
  Block operator()(Vector3i pos) const { return blocks[pos.x][pos.z][pos.y]; };
  Block operator()(std::int32_t x, std::int32_t y, std::int32_t z) const { return blocks[x][z][y]; }
