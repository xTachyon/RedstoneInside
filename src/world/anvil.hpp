...
public:
  ...
  struct ChunkInfo
  {
    ...
    ChunkInfo(std::uint32_t offset = 0, std::uint32_t  time = 0, std::uint8_t sectors = 0)
          : offset(offset), time(time), sectors(sectors) {}
  };
  
  enum class ChunkReadResult
  {
    OK,
    DoesntExists,
    Error
  ...
  Anvil(const boost::filesystem::path& path, const Vector2i& pos, bool create = true);
  ~Anvil();
  
  void open(const boost::filesystem::path& path, const Vector2i& pos, bool create = true);
  void close();
  void flush();
...
private:
  ...
  void createNewRegionIfOK(const boost::filesystem::path& filepath, bool create);
  void readHeader();
  void processHeader();
