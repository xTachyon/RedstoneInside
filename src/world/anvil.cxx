...
Anvil::Anvil(const std::string& path, const Vector2i& pos, bool create) : Anvil(fs::path(path), pos, create)
{}

Anvil::Anvil(const boost::filesystem::path& path, const Vector2i& pos, bool create)
{
...
void Anvil::open(const boost::filesystem::path& path, const Vector2i& pos, bool create)
{
  clear();
  position = pos;
  ...
  if (filesize % 4_KB != 0 || filesize < HeaderSize)
  {
    ...
    Logger::info((boost::format("%1% has an invalid size(%2% bytes). Renaming to %3% and trying to create a new one")
                  % path % filesize % newpath).str());
...
void Anvil::createNewRegionIfOK(const fs::path& filepath, bool create)
{
  if (!fs::exists(filepath))
  {
    if (!create)
    {
      throw std::runtime_error("Region file doesn't exists and set to not create: " + filepath.string());
    }
...
void Anvil::processHeader()
{
  ...
  for (std::int32_t i = 0; i < ChunksPerRegion; ++i)
  {
    ...
    {
      Logger::warn((boost::format("Chunk number %1% is out of range. Free sectors size : %2%. info.offset: %3%. info.sectors: %4%")
                    % i % freeSectors.size() % info.offset % info.sectors).str());
      writeChunkInfo(i, ChunkInfo());
      continue;
    }
    for (std::size_t j = info.offset, max = info.offset + info.sectors; j < max; ++j)
    {
      freeSectors[j] = false;
    }
  }
...
void Anvil::writeChunkInfo(const Vector2i& number, const Anvil::ChunkInfo& chunk)
{
...
void Anvil::writeChunkInfo(std::int32_t number, const Anvil::ChunkInfo& info)
{
  ...
  std::ofstream("a.b").write(reinterpret_cast<const char*>(header.data()), header.size());
}

Anvil::ChunkReadResult Anvil::readChunk(const Vector2i& number, ByteBuffer& buffer)
{
  return readChunk(getChunkNumberInRegion(number), buffer);
}

void Anvil::writeChunk(const Vector2i& number, const ByteBuffer& uncompresseddata)
{
...
Anvil::ChunkReadResult Anvil::readChunk(std::int32_t number, ByteBuffer& buffer)
{
  ...
  if (file.gcount() < 5)
  {
    Logger::error(std::to_string(file.gcount()) + " bytes read in Anvil.read. Expected at least 5");
  ...
  default:
  {
    Logger::error(std::to_string(static_cast<int>(buffer[4])) + " - unknown compression in Anvil.read. Expected 1 or 2.");
    return ChunkReadResult::Error;
  }
  }
