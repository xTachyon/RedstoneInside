...
void compressZlib(const ByteBuffer& in, ByteBuffer& out, CompressionLevel level, std::size_t start, std::size_t len)
{
  std::size_t size;
...
ByteBuffer decompressZlib(const ByteBuffer& data, std::size_t start, std::size_t len)
{
  ByteBuffer result;
  if (len == std::numeric_limits<std::size_t>::max())
  {
