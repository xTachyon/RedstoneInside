...
CommandListIterator
CommandManager::registerCommand(const CommandData& command) {
...
CommandListIterator CommandManager::registerCommand(CommandData&& command) {
  if (commandExists(command.command)) {
    throw std::runtime_error(
        "The command or one of it's aliases already exists");
  }
...
void CommandManager::unregisterCommand(const std::string& command) {
  mCommands.remove_if(
      [&](const CommandData& data) { return data.command == command; });
}
