...
Value& Value::operator[](const char* key) { return (*this)[std::string(key)]; }

void Value::assign(Tag&& tag) { *this = std::move(tag); }

Type Value::getType() const { return getType(data); }

Type Value::getType(const TagUniquePtr& ptr) {
  return ptr ? ptr->getType() : Type::End;
}

void Value::write(Serializer& s) const { data->write(s); }
