...
ByteBuffer& Serializer::operator()(const std::string& name,
                                   const TagCompound& root) {
  write(name, root);
  return buffer;
}
...
void Serializer::writeType(const Value& value) { writeType(value.getType()); }

void Serializer::write(const RootTag& root) { write(root.name, root); }
