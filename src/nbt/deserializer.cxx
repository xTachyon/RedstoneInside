...
void Deserializer::operator()(TagCompound& root) { read(root); }
...
std::string Deserializer::readString() {
  ...
  need(size);
  std::string s(buffer.as_const_char() + offset,
                buffer.as_const_char() + offset + size);
  offset += size;
  return s;
}
...
void Deserializer::read(RootTag& root) { read(root.name, root); }
